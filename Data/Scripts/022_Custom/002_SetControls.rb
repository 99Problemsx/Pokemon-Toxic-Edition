...
module Input
  class << self
    ...
    def buttonToKey(button)
      ...
      case button
      when Input::DOWN  then $PokemonSystem.game_control_code("Down")
      when Input::LEFT  then $PokemonSystem.game_control_code("Left") 
      when Input::RIGHT then $PokemonSystem.game_control_code("Right")
      when Input::UP    then $PokemonSystem.game_control_code("Up")
      when Input::ACTION then $PokemonSystem.game_control_code("Menu")    # Z, W, Y, Shift
      when Input::BACK   then $PokemonSystem.game_control_code("Cancel")  # X, ESC  
      when Input::USE    then $PokemonSystem.game_control_code("Action")  # C, ENTER, Space
      when Input::JUMPUP then $PokemonSystem.game_control_code("Scroll Up")
      when Input::JUMPDOWN then $PokemonSystem.game_control_code("Scroll Down")
      when Input::SPECIAL then $PokemonSystem.game_control_code("Ready Menu")
      else nil
      end
    end
  end
