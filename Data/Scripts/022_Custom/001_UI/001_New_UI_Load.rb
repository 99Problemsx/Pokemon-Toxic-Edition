...
class PokemonLoad_Scene
  ...
  def pbStartScene(trainer, frame_count, stats, map_id, screen)
    ...
    MenuHandlers.each_available(:load_screen, @screen) do |option, hash, name|
      ...
      commands.push(name)
      commands.push(hash)
      callback = Proc.new { |scene|
        pbPlayDecisionSE
        hash["effect"].call(@screen)
      }
  ...
  def pbStartScene2(backgroundinstant = false)
    if backgroundinstant
      ...
      pbFadeInAndShow([@backgroundsprites, @sprites]) { pbUpdate } 
    end
  end
  ...
  def pbEndScene
    ...
    pbFadeOutAndHide(@backgroundsprites) { pbUpdate }
...
class PokemonLoadScreen
  ...
  def pbStartLoadScreen
    # Get current map ID from the save data
    map_id = 0
    if !@save_data&.empty?
      map = @save_data[:map_factory].maps.first
      map_id = map&.map_id || 0
    end
...
MenuHandlers.add(:load_screen, :new_game, {
  ...
  "effect"    => proc { |menu|
    ...
    menu.pbEndScene
    Game.start_new
    $scene = pbCallTitle if $scene.nil?
  }
})
