...
def pbBerryPlantOrig
    ...
    if Settings::NEW_BERRY_PLANTS
        ...
        # Old mechanics
        return if !pbConfirmMessage(_INTL("It's {1} soil. Want to plant a berry?", (berry_plant.soil ? berry_plant.soil[:planting_description] : _INTL("soft, loamy"))))
        ask_to_plant = false
    end
        ...
        if berry
            ...
            if Settings::NEW_BERRY_PLANTS
                pbMessage(_INTL("The {1} was planted in the {2} soil.", 
                                GameData::Item.get(berry).name, (berry_plant.soil ? berry_plant.soil[:planting_description] : _INTL("soft, earthy"))))
...
def pbBerryPlant
    ...
    if $DEBUG && Input.press?(Input::CTRL) && berry_plant
        loop do
            cmds = [_INTL("Plant Info"),_INTL("Reset Plant"), _INTL("Continue")]
            help = [_INTL("View info about the plant event."), _INTL("Reset the entire plant (will clear all data)."), _INTL("Continue.")]
    ...
    if Settings::BERRY_PREFERRED_ZONE_WARNING && not_planted && berry_plant && 
        ...
        if berry_plant.plant_zone && GameData::BerryData.get(berry_plant.berry_id).preferred_zones.include?(berry_plant.plant_zone)
            pbMessage(_INTL("The {1} seemed happy to be planted here!", GameData::Item.get(berry_plant.berry_id).name))
        elsif berry_plant.plant_zone && GameData::BerryData.get(berry_plant.berry_id).unpreferred_zones.include?(berry_plant.plant_zone)
            pbMessage(_INTL("The {1} didn't seem happy to be planted here...", GameData::Item.get(berry_plant.berry_id).name))
        elsif berry_plant.soil && GameData::BerryData.get(berry_plant.berry_id).preferred_soil == berry_plant.soil[:id]
            pbMessage(_INTL("The {1} seemed happy to be planted here!", GameData::Item.get(berry_plant.berry_id).name))
        end
    end
end
